...
class SpeedBoostScript(scripts.Script):
                    ...
                    info_text = gr.HTML(
                        ...
                        <div style='padding: 10px; background-color: #1f2937; border-radius: 8px; margin-top: 10px;'>
                            ...
                            <div style='padding: 8px; background-color: #1e40af; border-radius: 6px; margin-bottom: 10px;'>
                                <p style='margin: 0; color: #60a5fa; font-weight: bold;'>🚀 NEW: Dynamic Step Optimization for Everyone!</p>
                                <p style='margin: 4px 0 0 0; font-size: 0.9em; color: #dbeafe;'>Now works in conservative mode with gentle step reduction</p>
                            </div>
                            <ul style='margin: 0; padding-left: 20px; color: #d1d5db;'>
                                <li><strong>Conservative Mode:</strong> Safe optimizations + gentle step reduction for high step counts</li>
                                <li><strong>FP16:</strong> Uses half-precision for ~60% speed increase (disable if errors occur)</li>
                                <li><strong>Inference Mode:</strong> Disables autograd + dynamic step optimization</li>
                                <li><strong>Memory Optimization:</strong> Reduces memory usage + enables step optimization</li>
                                <li><strong>Advanced:</strong> GPU-specific optimizations + aggressive step reduction</li>
                            </ul>
                            <p style='margin: 8px 0 0 0; font-size: 0.9em; color: #9ca3af;'>
                                💡 Conservative mode is recommended for stability. All modes now include step optimization for better speed!
                            </p>
                        </div>
                        """
                    )
    ...
    def process(self, p: StableDiffusionProcessing, 
        ...
        try:
            ...
            try:
                ...
                debug_print(f"Dynamic step optimization failed: {dynamic_e}", show_debug)
    ...
    def _apply_conservative_step_optimization(self, p: StableDiffusionProcessing, debug: bool):
        ...
        try:
            ...
            # Apply very conservative step reduction - only for high step counts
            # This ensures compatibility while still providing some speedup
            if original_steps > 40:
                # Only reduce steps if they're very high (40+)
                optimized_steps = max(25, int(original_steps * 0.8))
                debug_print(f"Conservative mode: High step count {original_steps} -> {optimized_steps} steps", debug)
            elif original_steps > 30:
                # Minimal reduction for medium-high step counts
                optimized_steps = max(20, int(original_steps * 0.85))
                debug_print(f"Conservative mode: Medium-high step count {original_steps} -> {optimized_steps} steps", debug)
            else:
                # No reduction for lower step counts in conservative mode
                optimized_steps = original_steps
                debug_print(f"Conservative mode: No step reduction for {original_steps} steps (safe range)", debug)
